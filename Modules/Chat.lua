...
function TOGBankClassic_Chat:OnCommReceived(prefix, message, _, sender)
    if IsInRaid() then
        if self.debug then TOGBankClassic_Core:Print('OnCommReceived: ignoring prefix', prefix, 'from', sender, '(in raid)') end
        return
    end
    ...
    if player == sender then
        if self.debug then TOGBankClassic_Core:Print('OnCommReceived: ignoring own message from', sender) end
        return
    end
        ...
        if not success then
            ...
            if data.type == "alt" then
                ...
                if sender == claimedNorm then
                    ...
                    local claimedIsBank = (TOGBankClassic_Guild and TOGBankClassic_Guild.IsBank) and TOGBankClassic_Guild:IsBank(claimedNorm) or false
                    if claimedIsBank then
                        if TOGBankClassic_Guild and TOGBankClassic_Guild.SenderHasGbankNote and TOGBankClassic_Guild:SenderHasGbankNote(sender) then
                            allowed = true
                        else
                            allowed = false
                        end
                ...
                if self.debug then TOGBankClassic_Core:Print('OnCommReceived: alt allowed=', tostring(allowed), 'from', sender, 'claimedNorm=', claimedNorm, 'claimedIsBank=', tostring(claimedIsBank)) end
                if allowed then
    ...
    if input == nil or input == "" then
        ...
        local commands = {
            ...
            ["debugdump"] = function ()
                ...
                if i==0 then TOGBankClassic_Core:Print('no entries') end
            end,
...
function TOGBankClassic_Chat:ReprocessQueue()
    TOGBankClassic_Core:ScheduleTimer(function (...) TOGBankClassic_Chat:OnTimer() end, 5)
end
