...
- ✅ [PERF-007] **CRITICAL** Bagnon execution timeout from BAG_UPDATE spam - Fixed ItemHighlight to throttle BAG_UPDATE event handling (500ms) and cache last search string to avoid redundant SEARCH_CHANGED signals. Previously every BAG_UPDATE (especially during zone changes) triggered RefreshHighlighting → UpdateBagnonHighlighting → SendSignal('SEARCH_CHANGED'), causing Bagnon to rebuild UI repeatedly and exceed execution time limit. Added throttling similar to Requests UI and search string comparison to only send signal when search actually changes. Result: Eliminated "Script from Bagnon has exceeded its execution time limit" errors on zone change. Location: ItemHighlight.lua Initialize event handler (~18-41), UpdateBagnonHighlighting search caching (~252-260), RefreshHighlighting/SetEnabled cache clearing (~367-377, ~95-105).
