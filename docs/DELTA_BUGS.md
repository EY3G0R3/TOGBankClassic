...
- ✅ [PERF-008] **CRITICAL** 5-second freeze on login/reload from synchronous RebuildBankerRoster() - Fixed by deferring RebuildBankerRoster() call to happen asynchronously 1 second after Guild:Init() completes. Previously called synchronously during GUILD_RANKS_UPDATE event immediately after login, causing RebuildBankerRoster() to loop through all guild members (500+) calling GetGuildRosterInfo(i) to check for "gbank" in notes, blocking main thread for 5+ seconds in large guilds. Also deferred in GUILD_ROSTER_UPDATE handler (0.5s delay). Result: Login/reload now responsive immediately, banker roster builds in background. Locations: Guild.lua Init (~276-281), Events.lua GUILD_ROSTER_UPDATE (~301-306).
