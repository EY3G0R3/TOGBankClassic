...
- ✅ [PERF-008] **CRITICAL** 5-second freeze on login/reload from synchronous roster operations - Fixed by deferring ALL expensive roster operations (RefreshOnlineCache and RebuildBankerRoster) to run asynchronously after login completes. Previously called synchronously during GUILD_RANKS_UPDATE and GUILD_ROSTER_UPDATE events immediately after login. RebuildBankerRoster loops through all guild members (500+) calling GetGuildRosterInfo(i) to check for "gbank" in notes, and RefreshOnlineCache also loops through all members. Combined, these blocked main thread for 5+ seconds in large guilds. Fixed in THREE paths: (1) Guild:Init() defers RebuildBankerRoster with 1s delay when data exists, (2) Guild:Reset() defers RebuildBankerRoster with 1s delay on first load/after wipe, (3) GUILD_ROSTER_UPDATE defers both RefreshOnlineCache and RebuildBankerRoster with 0.5s delay. Result: Login/reload now responsive immediately, roster operations build in background. Locations: Guild.lua Init (~282-287), Reset (~259-265), Events.lua GUILD_ROSTER_UPDATE (~295-330).
