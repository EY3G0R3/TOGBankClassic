...
- ✅ [FULFILL-003] **LOW** Fulfill button showed blank red button for "in mail and bank" state — `FULFILL_ICON_IN_MAIL_AND_BANK` was set to `"|TInterface\\Icons\\INV_Misc_Chest_01:18:18:0:0|t"`. `INV_Misc_Chest_01` does not exist as a texture in WoW Classic Era; the engine renders a blank red fallback square in place of any missing texture. Fix: Replaced the single missing texture with two confirmed-working icons rendered side-by-side at reduced size: `INV_Misc_Bag_07` (bag, 14px) + `INV_Letter_06` (wax letter, 14px). The paired icons visually communicate "bank AND mail" using textures already proven working in other states. Location: UI/Requests.lua `FULFILL_ICON_IN_MAIL_AND_BANK` constant (~line 37).
- ✅ [FULFILL-002] **MEDIUM** Fulfill button showed `?` icon and "Need X more items" when bags had some but not enough of the requested item — `CanFulfillRequest` in Mail.lua only checked `alt.mail.items` and `alt.bank.items` when `totalInBags == 0`. When the player had a partial quantity in bags (e.g. 125 linen with a request for 150), `totalInBags = 125 > 0` caused the mail/bank check to be skipped entirely. `CalculateFulfillmentPlan` returned a generic shortage reason that fell into the final catch-all `?` branch. The UI gave no indication that more items were available nearby to complete the order. Fix: Added a post-plan shortfall check: when `not plan.canFulfill and totalInBags < qtyNeeded`, check both `altData.mail.items` and `altData.bank.items` using the shared `hasMatchInTable` helper and return three new reason strings: `"shortfall in bank and mail"`, `"shortfall in mail"`, `"shortfall in bank"`. Both icon branches in `_PopulateRow` and `_RefreshFulfillButtons` handle all three new reasons with contextual tooltip text showing current bag count and target quantity (e.g. "Have 125 in bags. More available in your bank and mail inbox — pick up or retrieve the rest to reach 150."). Locations: Mail.lua `CanFulfillRequest` post-plan block; UI/Requests.lua `_PopulateRow` and `_RefreshFulfillButtons` icon branches.
- ✅ [FULFILL-001] **MEDIUM** Fulfill button icon stayed stuck on split (shovel) icon after a bag split completed — The `UNIT_BAG_UPDATE` handler in `UI/Requests.lua` called `DrawRows(actor, isActorBank)`, which skips any row that is not marked dirty. After a split, the row was already drawn (not dirty), so `DrawRows` was a no-op and the icon never transitioned from `FULFILL_ICON_NEED_SPLIT` to its post-split state. `_RefreshFulfillButtons` already existed for exactly this purpose (per its own header comment) and re-evaluates icon state on all visible rows regardless of the dirty flag. Fix: Replaced the `DrawRows(actor, isActorBank)` call in `OnBagUpdate` with `_RefreshFulfillButtons(actor, isActorBank, mailboxOpen)`. Location: UI/Requests.lua `OnBagUpdate` handler.
- ✅ [HIGHLIGHT-001] **LOW** Bagnon bag search filter did nothing for recipe/pattern items (e.g. "Pattern: Ironfeather Breastplate") — `UpdateBagnonHighlighting` in `ItemHighlight.lua` built the Bagnon search string by concatenating item names with `|` (Bagnon's OR operator). For the `neededItemIDs` loop it called `C_Item.GetItemNameByID` and inserted the raw name directly, which for recipe items includes the craft-type prefix (`"Pattern:"`, `"Formula:"`, `"Plans:"`, etc.). Bagnon's search parser does not support the `:` character — the presence of a colon caused the entire search term for that item to be silently ignored, so recipe items in the bags were never highlighted. The `neededItems` (name-keyed legacy) loop had the same latent issue. Fix: Added a shared `stripRecipePrefix` helper that strips all known Blizzard craft prefixes (`Formula:`, `Pattern:`, `Recipe:`, `Plans:`, `Schematic:`, `Design:`, `Manual:`) via `string.gsub`. Both the `neededItems` and `neededItemIDs` loops now pass names through this helper before appending to the Bagnon search string. Location: ItemHighlight.lua `UpdateBagnonHighlighting` — `stripRecipePrefix` helper + both name-append loops.
- ✅ [TOOLTIP-001] **MEDIUM** Item link tooltip displayed banker info for non-guild bankers — `OnTooltipSetItem` in `TooltipBankerInfo.lua` iterated all entries in `info.alts` with no guild membership check. Any banker persisted in the local database — regardless of guild affiliation — would appear in the "Bankers:" tooltip section when hovering an item link. This could surface data from bankers on other guilds or characters that had since left the guild. Fix: Added `TOGBankClassic_Guild:IsInCurrentGuildRoster(altName)` check as a combined `and` condition on the existing `alt.items` guard, so only alts confirmed present in `memberRoster` (O(1) lookup) are eligible for display. Two earlier fix attempts were reverted: (1) a `goto continue` / `::continue::` pattern — not valid Lua 5.1 syntax used by WoW Classic Era, causing a silent Lua error that killed the entire tooltip hook; (2) a call to the non-existent `IsGuildMember` method — also a runtime error. Final fix uses a plain `and` condition which is fully Lua 5.1 compatible and requires no control-flow changes. Location: Modules/TooltipBankerInfo.lua `OnTooltipSetItem` loop (~line 27).

**Recent Fixes (2026-04-04):**
- ✅ [REQ-002] **MEDIUM** Requests UI item tooltip showed wrong same-name variant — The `OnEnter` handler on the item column EditBox overlay searched `info.alts` by display name (`item.Info.name`) and took the first match. Since all class-specific variants of "Punctured Voodoo Doll" share an identical name, the tooltip always resolved to whichever variant `pairs()` iterated first — typically the Warrior doll — regardless of which variant was actually requested. This meant a player who requested a Druid doll would see "Punctured Voodoo Doll (Druid)" written in the request row text but the tooltip would show the Warrior variant's stats and color. Root cause: the `OnEnter` handler had no access to `request.itemID`; it only stored `_itemName` on the EditBox frame. Fix: Store `_itemID = req.itemID` on the EditBox frame alongside `_itemName` when populating each row. In `OnEnter`, when `_itemID` is non-nil, search `info.alts` by exact `item.ID == requestItemID` match to retrieve the correct item link; fall back to `item:ID` hyperlink if no inventory entry is found (e.g. item was since spent). When `_itemID` is nil (legacy request without itemID), retain the existing name-based first-match fallback. Location: UI/Requests.lua `EnsureRowForRequest` item column EditBox `OnEnter` handler (~line 1435) and row population block (~line 1970).
- ✅ [REQ-001] **HIGH** Same-name item variants (e.g. "Punctured Voodoo Doll" Priest vs Warrior) were indistinguishable in the request system — All class-specific Punctured Voodoo Doll variants share the identical display name returned by `GetItemInfo`, but each has a unique numeric item ID. The request system stored only `request.item` (the name string). Every subsystem that consumed requests — `CountItemInBags`, `FindItemsByName`, `CanFulfillRequest`, `PrepareFulfillMail`, `BuildNeededItemsList`, `IsItemNeeded`, and `CheckMailFulfillment` — matched solely by name, making all class variants identical from the system's perspective. A banker holding only a Warrior Voodoo Doll would appear able to fulfill a Priest request (and vice versa), item-highlight bag scanning would grey-out the wrong slots, and mail fulfillment detection would credit the wrong variant. Fix: Threaded `request.itemID` (numeric Blizzard item ID) through the entire request lifecycle with full backward compatibility. **(1) Wire format** (`RequestLog.lua`): `serializeRequestV1` appends `req.itemID or false` as `arr[12]`; `deserializeRequestV1` reads `itemID = tonumber(arr[12]) or nil`; `sanitizeRequest` preserves `itemID = tonumber(req.itemID) or nil`. Old senders produce messages without `arr[12]` → `tonumber(nil) = nil` → all paths fall back to name-matching automatically. **(2) Request creation** (`UI/Search.lua`): `ShowRequestDialog` sets `requestContext.itemID = itemEntry.ID`; `SubmitRequest` includes `itemID = self.requestContext.itemID` in the submitted request table. **(3) Bag scanning** (`Bank.lua`): `FindItemsByName` and `CountItemInBags` accept an optional `itemID` second argument; when non-nil, match is performed by `itemInfo.itemID == targetID` (exact, handles variants); when nil, falls back to `GetItemInfo(hyperlink)` name comparison (legacy). **(4) Mail fulfillment** (`Mail.lua`): both `CanFulfillRequest` and `PrepareFulfillMail` now pass `request.itemID` to `CountItemInBags`. **(5) Bag highlighting** (`ItemHighlight.lua`): added parallel `neededItemIDs` table keyed by numeric ID for new requests; `BuildNeededItemsList` routes to `neededItemIDs[request.itemID]` when set, `neededItems[itemName]` for legacy; `IsItemNeeded` checks the ID table first then falls back to the name table; `UpdateBagnonHighlighting` builds Bagnon search terms from both tables with `seenNames` deduplication; `UpdateDefaultBagHighlighting` and `UpdateBankHighlighting` pass `itemInfo.itemID` to `IsItemNeeded`. **(6) Pre-existing bug fixed**: `BuildNeededItemsList` was computing `qtyNeeded` as `request.quantity - request.quantityFulfilled`, but the field is `request.fulfilled` — `quantityFulfilled` never existed in the schema, meaning `qtyNeeded` was always equal to `request.quantity` (never subtracted already-fulfilled amounts). Fixed to use `request.fulfilled`. The `itemID` field survives all network receive paths: `togbank-rd2` positional records (`ReceiveRequestsByIdV1` → `deserializeRequestV1`), mutation broadcasts (`ReceiveRequestMutations` → `ApplyRequestMutation` → `mergeRequest` → `sanitizeRequest`), and snapshots (`ApplyRequestSnapshot` → `mergeRequest` → `sanitizeRequest`). The legacy `togbank-rd` key-value path is unaffected — it carries no `itemID` field, so `sanitizeRequest` returns `nil` and all consumers fall back to name-matching. Locations: RequestLog.lua `serializeRequestV1` (~line 68), `deserializeRequestV1` (~line 275), `sanitizeRequest` return (~line 405), `CheckMailFulfillment` (~line 2148), wire-format comments (~lines 55-59, 1488); UI/Search.lua `ShowRequestDialog` requestContext (~line 220), `SubmitRequest` (~line 290); Bank.lua `FindItemsByName` (~line 375), `CountItemInBags` (~line 415); Mail.lua `CanFulfillRequest` (~line 50), `PrepareFulfillMail` (~line 180); ItemHighlight.lua `neededItemIDs` state (~line 10), `BuildNeededItemsList` (~line 136), `IsItemNeeded` (~line 192), `UpdateBagnonHighlighting` (~line 255), `UpdateDefaultBagHighlighting` (~line 330), `UpdateBankHighlighting` (~line 360).

**Recent Fixes (2026-04-03):**
- ✅ [MAIL-014] **MEDIUM** Order fulfillment sound never played — `PlaySound("AuctionWindowClose")` was called when a mail arrived fulfilling an active request, but string-based sound names were removed from the WoW API in Patch 7.3.0 (Legion). Classic Era uses the same numeric-ID-only API; passing a string silently does nothing with no Lua error. Fix: Replaced the string argument with `SOUNDKIT.AUCTION_WINDOW_CLOSE` (the standard FrameXML constant, resolves to the correct numeric SoundKitID) with `11561` as a numeric fallback in case the `SOUNDKIT` table is unavailable. Additionally added a new per-character Option toggle "Play Sound on Order Fulfilled" (Options > TOGBankClassic, order 2.8, defaults on) so players who find the sound disruptive can disable it without losing the notification. Locations: Mail.lua `OnMailShow` fulfillment detection block (~line 460); Options.lua toggle definition (~order 2.8) + `IsOrderFulfillmentSoundEnabled()` getter.

**Recent Fixes (2026-04-02):**
- ✅ [UI-017] **LOW** One row in the Requests UI table randomly appeared much taller (larger text) than all other rows — AceGUI's Table layout determines each row's height by calling `label.label:GetStringHeight()` after `label:SetWidth()` is applied. When a cell's FontString has word wrap enabled (the default), `GetStringHeight()` returns the full multi-line height if the text overflows the column width — e.g. a long item name like "Enchanted Thorium Breastplate" in the Item column, or a date string decorated with an icon prefix in the Date column, could wrap to two lines and report 36px instead of 18px. The Table layout allocated that height for the entire row, making every cell in that row appear vertically spread out and its text visually larger relative to adjacent single-line rows. Because only the widest/longest cell in a given row triggers wrapping and other rows happen to fit on one line, the effect was sporadic — most rows looked normal while one outlier appeared oversized. Fix: Added `label.label:SetWordWrap(false)` to the label creation block in `EnsureRowForRequest`, applied to all non-actions columns. `GetStringHeight()` now always returns exactly one line height (18px) regardless of text length; oversized text is clipped rather than wrapped. The transparent EditBox overlay on the Item column remains fully copyable. Location: Modules/UI/Requests.lua `EnsureRowForRequest` label creation block (~line 1321).
- ✅ [UI-016] **MEDIUM** Copyable item text in Requests UI — six iterative bugs across two sessions. **(1) Text invisible:** Initial EditBox inside a `SimpleGroup` rendered no visible text regardless of `SetFontObject`/`SetTextColor`/`SetText`; root cause: WoW EditBox does not reliably render in all frame hierarchy contexts. **(2) Text still invisible after AceGUI pivot:** Replaced container with AceGUI Label + `SetAllPoints(label.label)` overlay — EditBox still blank. **(3) Stacked/duplicate text:** Debug workaround set both `label:SetText(colorize(...))` AND `label.editbox:SetText(cellVal)` simultaneously — text appeared via the label but was visually doubled. **(4) Regression after debug removal:** Calling `label:SetText("")` collapsed AceGUI frame height, hiding the row. **Final architecture (session 1):** EditBox is a permanently transparent overlay — Label renders all visible text always; `OnEditFocusGained` populates EditBox + `HighlightText()` for Ctrl+C; `OnEditFocusLost` clears EditBox. **(5) White EditBox text overlapping colored label on click (session 2):** On focus gain the EditBox text rendered visibly in white on top of the colored Label. Fix: `SetAlpha(0)` on the EditBox frame; also added `SetHighlightColor(0, 0, 0, 0)` to zero the selection highlight that remained visible on drag-select. **(6) Focus permanently stuck after clicking:** WoW does not auto-release EditBox focus when clicking outside a custom frame; cursor remained trapped indefinitely. Fix: `OnEditFocusGained` starts `C_Timer.After(5, ...)` that calls `ClearFocus()` if still focused (5 s is enough to Ctrl+C); `OnKeyDown` calls `ClearFocus()` immediately on Escape. Location: Modules/UI/Requests.lua — widget creation block (~line 1368) and population block (~line 1930).

**Recent Fixes (2026-04-01):**
- ✅ [P2P-032] **HIGH** Delivery watchdog (60s) shorter than AceCommQueue drain time under load — `OnSyncAccept` started a `C_Timer.After(60, ...)` delivery watchdog that called `OnFailed(sessionId, "delivery_timeout")` if data had not arrived within 60 seconds of the peer sending sync-accept. AceCommQueue-1.0 serialises outbound sends per `(prefix, dist, target)` key and drains through ChatThrottleLib at ~800 bytes/sec BULK. Under the 3-slot concurrent send cap, the third queued whisper target can legitimately take 65–70 seconds wall-clock time to drain a 9 KB payload (observed: "~38 chunks, 9449 bytes in 68.1s" and "~16 chunks, 3899 bytes in 69.5s"). The 60s watchdog fired before delivery completed → `OnFailed` decremented `activeSessions`, cleared the session, and marked the requester as exhausted in `triedPeers` for that alt → `ScheduleCatchUp` fired a 45s deferred rebroadcast → when the delayed alt data finally arrived (a few seconds after the 68s drain), `OnAltCompleted` found `sessionsByAlt[norm] = nil` and was a no-op → the re-broadcast cycle restarted the whole sync unnecessarily, keeping sends queued far longer than needed. The `SEND_TIMEOUT = 90s` safety-release timer was also shorter than worst-case drain, meaning a send slot could be auto-released while a legitimate transfer was still in flight, allowing a 4th concurrent send to sneak in past the cap. Fix: Introduced named constant `DELIVERY_TIMEOUT = 180s` (replaces hardcoded `60` in `OnSyncAccept`); bumped `SEND_TIMEOUT = 210s` (must always exceed `DELIVERY_TIMEOUT` to prevent the safety-release from racing the delivery watchdog). Both values chosen to comfortably exceed the observed 68-70s worst-case. Location: P2PSession.lua constants block + `OnSyncAccept` delivery watchdog timer.
- ✅ [P2P-031] **HIGH** Old pull-based `togbank-r` responder running concurrently with P2P session responder, each with independent send caps — The `togbank-r` alt-request handler (Chat.lua) was the **responder** side of the defunct pull-based sync system (System 1). P2P-026 removed all initiator call sites (`QueryAltPullBased`) but left the responder fully live. Old clients (or clients that hadn't reloaded since P2P-026) could still broadcast `togbank-r alt-request` messages to GUILD; any peer with matching data would answer through the old handler, incrementing `pendingSendCount` / `MAX_PENDING_SENDS = 3`. Simultaneously, P2P session sync-requests from `HandleSyncRequest` incremented `P2P.activeSends` / `MAX_ACTIVE_SENDS = 3`. The two counters were completely independent — sharing no state — producing a real-world ceiling of 6+ concurrent outbound data streams. Observed symptom: 20–30 simultaneous "Sharing guild bank data…" messages, completions arriving 60–150s after start, visible in chat log pasted 2026-04-01. This was not present before P2P-026 because the initiator removal left the responder side unchecked — the bug was introduced as a side-effect of that cleanup rather than a pre-existing issue. Fix: (A) Extracted `TryAcquireSendSlot(requester)` and `ReleaseSendSlot(requester, reason)` helpers from `HandleSyncRequest` into P2PSession.lua. `TryAcquireSendSlot` atomically checks `P2P.activeSends` total against `MAX_ACTIVE_SENDS`, increments if under cap, and sets the 90-second safety-release timer; returns false if at cap. `ReleaseSendSlot` is idempotent (guarded by `> 0`). (B) `HandleSyncRequest` now calls `TryAcquireSendSlot`/`ReleaseSendSlot` instead of inlining the slot logic. (C) The `togbank-r` responder's capacity check replaced `pendingSendCount >= MAX_PENDING_SENDS` with a sum of `P2P.activeSends`. After acquiring a slot the old 30-second `pendingSendCount++` + timer is replaced with a single `TryAcquireSendSlot(sender)` call (same 90-second safety release). (D) `CreateOnChunkSentCallback` in Guild.lua now accepts a `requester` argument and calls `P2P:ReleaseSendSlot(requester)` on completion for whisper sends; falls back to the legacy `pendingSendCount--` only for GUILD-broadcast sends (banker self-share, no target). Both paths now compete for the single shared 3-slot cap in `P2P.activeSends`. Locations: P2PSession.lua `TryAcquireSendSlot` + `ReleaseSendSlot` (new), `HandleSyncRequest` (refactored); Guild.lua `CreateOnChunkSentCallback` (new `requester` param + `ReleaseSendSlot` call), `SendAltData` (pass `distTarget` to callback); Chat.lua `togbank-r` peer responder capacity check + backoff timer.
...
- ✅ [ALIAS-001] **LOW** `/bank` routed through bare `Toggle()` stub instead of full `ChatCommand` handler after alias feature was added — Remote commit `944672f` introduced `RegisterAliasCommands()`, which registers `/bank` and `/gbank` through the `ChatCommand` dispatcher (argument parsing, unknown-command handling, `registerBankCommand`/`registerGbankCommand` Options-toggle guards). However, `Chat:Init()` still contained the original hardcoded `self:RegisterChatCommand("bank", function() self:Toggle() end)` that predated the feature. Both registrations were active simultaneously: WoW's slash handler table is last-writer-wins, and `Init()` runs before `RegisterAliasCommands()`, so the alias registration correctly overwrote it at runtime — but the old registration was misleading dead weight and would silently win on any path where `RegisterAliasCommands()` was skipped or ordering changed. The old handler called `Toggle()` with zero arguments, bypassing `ChatCommand` entirely and ignoring all Options toggles. Fix: Removed the hardcoded `self:RegisterChatCommand("bank", ...)` from `Init()`; added a comment: `-- /bank and /gbank are registered after Options init via RegisterAliasCommands()`. `/bank` is now exclusively owned by `RegisterAliasCommands()`. Location: Chat.lua `Init` (~line 67, commit 3244125).
- ✅ [SEND-001] **LOW** Send-progress display always showed "1 chunks, 0.0s" regardless of actual message size or transfer time — `CreateOnChunkSentCallback` was written assuming AceComm's `callbackFn` fires once per wire chunk (similar to ChatThrottleLib's raw chunk callbacks). In practice, `AceCommQueue-1.0` intercepts all intermediate chunk callbacks and delivers only the final callback once `bytesSent >= totalBytes`. Two display bugs resulted: (1) `sendStats.startTime` was initialized to `nil` and set to `GetTime()` on the first callback invocation — but since only one invocation ever fires (at send completion), `elapsed = GetTime() - startTime` was always measured within the same frame, producing 0.0s. (2) `sendStats.chunksSent` incremented once per invocation, so it was always 1. Additionally, the "Sharing guild bank data: N bytes in ~M chunks..." progress message was guarded by `if chunksSent == 1` inside the callback, meaning it fired simultaneously with "Send complete" at the end of the transfer rather than at the start. Fix: (1) Capture `startTime = GetTime()` at closure creation (just before `SendCommMessage`), so elapsed spans the full CTL drain time. (2) Replace `sendStats.chunksSent` in the completion summary with `math.ceil(totalBytes / 254)` (estimated chunks, same formula as the progress line). (3) Move the "Sharing guild bank data..." message to `SendAltData` immediately before `SendCommMessage`, so it fires at transfer start. Removed now-unused `lastBytes` and `chunksSent` fields from `sendStats`. No behavioral change to the send path; display-only fix. Location: Guild.lua `CreateOnChunkSentCallback` (~line 2356), `SendAltData` (~line 2574, commit f9ebd49).
...
- ✅ [P2P-026] **CRITICAL** Two sync systems running simultaneously causing race conditions and data corruption — System 1 (old pull-based/HLR: `QueryAltPullBased` → GUILD broadcast → `togbank-rr` ACK → "will send delta") was still active alongside System 2 (new hash-offer P2P: `SyncDeltaVersion` → hash-list broadcast → collect window → Dispatch → whispered sync-requests). Both systems operated on the same `togbank-state`/`togbank-d4` delivery pipeline. System 1 was uncapped (each alt broadcasted independently to GUILD, any peer could respond, no `MAX_ACTIVE_SESSIONS` guard) while System 2 was capped at 3. During a collect window, System 1's `RequestHashListFromBanker` path was not guarded by `isCollecting`, so GUILD broadcasts could fire while System 2 was accumulating hash-offers, leading to concurrent deliveries for the same alt from different peers. `ApplyDelta` last-write-wins based on `updatedAt` which mitigated data loss but the race was real and caused stalls. Fix: Removed all 5 external call sites of `QueryAltPullBased`: (1) `DeltaComms.lua:1732` (FastFillMissingAlts hash=0 alt) — dropped; hash=0 alts now broadcast hash=0 via System 2's collect cycle. (2) `Guild.lua:1074` (RequestHashListFromBanker no-banker + no-hash path) — replaced with `ScheduleCatchUp("no_hash_no_banker")`. (3) `Chat.lua:645` (version-broadcast handler, hash=0 alt) — replaced with `ScheduleCatchUp("version_no_hash")`. (4) `Chat.lua:1273` (BroadcastP2PRequest 5s timeout, no peer responded) — replaced with `ScheduleCatchUp("p2p_no_response")`. (5) `Chat.lua:2116` (HLR missingContent handler, no hash) — replaced with `ScheduleCatchUp("hlr_missing_hash")`. `QueryAltPullBased` function body remains in Guild.lua as dead code but is no longer reachable. Only System 2 (`SyncDeltaVersion` → hash-offer → Dispatch) now drives all sync. Locations: DeltaComms.lua (~1725), Guild.lua (~1074), Chat.lua (~645, ~1273, ~2116).
- ✅ [P2P-027] **HIGH** `ScheduleCatchUp` cycle counter burned through all 5 cycles in ~3 seconds on first-run / wipe — On a blank DB (first install or `/togbank wipe`), `RequestHashListFromBanker`'s inner loop iterates all 36 roster alts. Each alt with no local hash called `ScheduleCatchUp("no_hash_no_banker")` independently. `C_Timer.After` returns nil in Classic Era, so `self.catchUpTimer = C_Timer.After(...)` set the timer handle to nil. The guard `if self.catchUpTimer then return end` was always false (nil = falsy), allowing every alt to increment `catchUpCycles` and scheduled timers that were never tracked. 36 alts = 36 calls = 6 groups of 5 cycles exhausted per group, resetting between groups, all within 3 seconds. No 45-second delay ever fired. Fix: Set `self.catchUpTimer = true` before calling `C_Timer.After(...)` so the guard fires correctly regardless of what the API returns. The C_Timer callback still clears `catchUpTimer = nil` on expiry. Result: only the first call in any burst schedules a catch-up; all subsequent calls in the same frame return immediately. Location: P2PSession.lua `ScheduleCatchUp` (~line 96).
- ✅ [P2P-028] **CRITICAL** First-run / wipe: hash-list broadcast empty, `SyncDeltaVersion` returned early, no data ever received — On a blank DB, every alt has `inventoryHash=0`, `version=0`, `mailHash=0` and no items. `BuildBankerHashList` skipped all of them via the ROSTER-002 guard (`if not hasAnyData then skip`). Result: `BuildBankerHashList` returned an empty table → `SyncDeltaVersion` saw `altCount=0` → early return → no broadcast → no hash-offers → no collect window → no Dispatch → `ScheduleCatchUp` fired → same empty list → 5 cycles wasted → complete failure to receive any data. ROSTER-002 was originally intended to suppress phantom entries seeded by broadcasters who had never opened a banker, but it blocked the legitimate wipe-recovery signal. Fix: Removed ROSTER-002 skip guard from `BuildBankerHashList`. All roster alts are now always included, even with hash=0. Broadcasting hash=0 is the correct signal: peers compare their stored hash (non-zero) vs received hash (0) → they have newer data → they send hash-offers. Peers who also have hash=0 for an alt won't offer. Backwards-compatible: no protocol change. Location: Guild.lua `BuildBankerHashList` (~line 800).
- ✅ [P2P-029] **HIGH** `activeSessions` counter exceeded `MAX_ACTIVE_SESSIONS` (3) cap due to dispatch-time vs accept-time race — `activeSessions` was only incremented in `OnSyncAccept` (when peer replied with sync-accept), but `DispatchList` used `activeSessions` to calculate available slots before any accepts had arrived. Timeline: Dispatch fires 3 sessions → activeSessions=0 → all 3 dispatched. Session A completes → FlushPendingDispatch → activeSessions still 0 (A was ACTIVE, decremented to 0) → 2 more dispatched. Meanwhile B and C accept → activeSessions now 2. A second collect window Dispatch fires 3 offers → slots = MAX(3) - 2 = 1 → dispatches 1 → queues 2. Sequence of concurrent flushes each reading stale counts drove activeSessions to 5 in observed logs. The pending queue then never drained because `activeSessions` stayed inflated — slots < 0 at every FlushPendingDispatch call. Fix: (1) Increment `activeSessions` in `DispatchList` at session creation (STATE.DISPATCHED), not in `OnSyncAccept`. (2) Remove the `activeSessions++` from `OnSyncAccept` entirely. (3) Decrement `activeSessions` unconditionally in both `OnAltCompleted` and `OnFailed` (removing the `if s.state == STATE.ACTIVE` guard) since a slot is reserved from dispatch time. `activeSessions` now accurately represents "sessions in flight" (DISPATCHED + ACTIVE) at all times. Location: P2PSession.lua `DispatchList` (~line 267), `OnSyncAccept` (~line 320), `OnAltCompleted` (~line 418), `OnFailed` (~line 436).

**Recent Fixes (2026-03-28):**
- ✅ [P2P-023] **CRITICAL** Hash-list broadcast collisions causing INTEGRITY-MISMATCH errors — When the same banker broadcast two hash-lists before the first transmission completed, AceComm's multipart message reassembly corrupted the second broadcast. AceComm keys partial message spools by `"prefix\tdistribution\tsender"` — identical for concurrent broadcasts from the same sender. Timeline: Broadcast #1 starts at T+0 (9KB message = 35 chunks over 11 seconds via ChatThrottleLib 800 byte/sec throttle) → Broadcast #2 starts at T+5 (manual `/togbank hashupdate` or login event) → Receiver's AceComm processes: FIRST(msg1), NEXT(msg1), NEXT(msg1), FIRST(msg2) [overwrites spool[key]], NEXT(msg1), NEXT(msg1), LAST(msg1) → Reassembled message = FIRST(msg2) + NEXT/LAST(msg1) → CRC fails ("stop=PASS crc=FAIL"). Symptom: 14 reported INTEGRITY-MISMATCH logs, all from `togbank-hl`, message sizes 800–9300 bytes (3–36 chunks). Same bankers appeared multiple times (Cardsngames, Monsterwound, Pimptasty), indicating systematic issue not random corruption. Root cause: No sender-side cooldown guard. Three broadcast triggers: (1) periodic timer (every 3 minutes), (2) manual `/togbank hashupdate` command, (3) login/zone events. Manual commands could fire during timer in-flight, login events could fire during zone-in cooldown period (PERF-021 allows NORMAL priority through), creating collision window. AceComm's `OnReceiveMultipartFirst` has commented-out `lostdatawarning` (line ~171) indicating library developers knew about this but left it silent. Fix: Added `hashBroadcastInProgress` guard flag shared across `Events:SyncDeltaVersion` and `Guild:HashUpdate`. Before sending, check if flag is set: (1) BULK priority (periodic timer) → skip broadcast with debug log (next 3-minute timer will catch changes), (2) NORMAL/ALERT priority (login, manual commands) → defer 16 seconds with retry (max 3 attempts = 48s total), force through after 3 retries to prevent indefinite blocking. After `SendCommMessage`, set flag to true and start 15-second timeout (covers 9KB message at 11s + margin). Telemetry: `hashBroadcastCount` tracks total broadcasts sent, `hashBroadcastBlocked` tracks collision prevention count. Display via `/togbank deltastats` in "Protocol Health" section showing `X sent, Y blocked (Z% collision rate)`. Debug tag: `COLLISION-GUARD` under PROTOCOL category shows skip/defer/retry/force-through decisions in real-time. Backwards compatible: sender-side only, no protocol changes, old clients benefit from fixed senders. Locations: Events.lua `SyncDeltaVersion` (~242-272 collision guard, flag management at ~280-286); Guild.lua `HashUpdate` (~3372-3380 collision check, flag management at ~3393-3398); Chat.lua `PrintDeltaStats` "Protocol Health" section (~2951-2957 telemetry display); Constants.lua DEBUG_TAGS PROTOCOL section (~69 COLLISION-GUARD tag registration).
