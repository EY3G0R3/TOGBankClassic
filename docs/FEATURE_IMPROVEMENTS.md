...
- [ ] **Reset database button** - Add a button in the Options UI (or a dedicated danger zone panel) that wipes all saved guild bank data; must show a yes/no confirmation popup before proceeding; equivalent to `/togbank wipe` but accessible without knowing slash commands
...
- [x] ~~**Cancel reason dialog**~~ **IMPLEMENTED (March 30, 2026): Clicking Cancel on a request now opens an AceGUI reason-selection dialog instead of cancelling immediately; banker selects from three preset reasons (item unavailable, quantity exceeds guild policy %, item is on a different banker); selected reason is stored in `req.notes` and broadcast guild-wide; the cancel reason %  syncs live with the officer `maxRequestPercent` setting; cancelled rows show the reason text in the date-column tooltip below the Cancelled timestamp**
