...
- [x] ~~Add mouseover tooltip for truncated item names to show full text~~ **IMPLEMENTED (April 2, 2026): Hovering any item name in the Requests window shows the full WoW item tooltip with stats, quality, and level requirements; uses SetHyperlink with the stored item link, falling back to "item:ID" if only the item ID is available; tooltip anchors to the right of the item name**
- [x] ~~**Copyable item text in Requests UI**~~ **IMPLEMENTED (April 2, 2026): Item name column in Requests window uses a transparent EditBox overlay on top of the Label; clicking an item name highlights the text for Ctrl+C copying; typing is blocked (restores original text); Escape clears focus; text color still reflects request status (red=cancelled, green=fulfilled/complete, white=open)**
...
- [x] ~~**Order fulfillment notification**~~ **IMPLEMENTED (April 3, 2026): Plays a sound (`SOUNDKIT.AUCTION_WINDOW_CLOSE`) when mail arrives that fulfills an active request; chat notification displayed with item name, quantity, sender, and fulfilled/remaining count; sound and notification fire only when `CheckForFulfilledRequest` matches a live request. Bug fixed: original `PlaySound("AuctionWindowClose")` string-based call was silently ignored since WoW Patch 7.3.0 — replaced with numeric SoundKitID. New Options toggle "Play Sound on Order Fulfilled" (defaults on) added so players can disable the sound without losing chat notification. Location: Mail.lua fulfillment detection block; Options.lua toggle + `IsOrderFulfillmentSoundEnabled()` getter**
...
- [x] ~~**Reset database button**~~ **IMPLEMENTED (April 3, 2026): Reset Database button exists in Options > TOGBankClassic main tab; now has a proper tooltip describing what it does, that it's irreversible, and that it's equivalent to `/togbank wipe`. Note: confirmation popup not yet added — button fires immediately on click**
- [~] **Banker data freshness indicator** - ~~Add a button that runs the equivalent of `/togbank hashdebug` and displays the results in a user-friendly popup~~ **v0.9.12 PARTIAL: Banker tabs in the inventory window turn red when that banker is HLR-pending (same definition as `/togbank hashdebug`); hovering a red tab shows a tooltip explaining newer data exists. Uses `Guild:IsAltSyncPending` as single source of truth. Remaining: dedicated button/popup UI showing full hashdebug report for non-dev users**
- [ ] **Restructure `/togbank hashdebug` output** - Rewrite output to be human-readable for non-developers; replace raw hash values with plain-language status (e.g. "Toon — up to date", "Toon — data may be outdated (last seen 3 hours ago)"); keep raw hashes only in debug-level output
- [ ] **Delta stats button** - Add a button that runs the equivalent of `/togbank deltastats` and displays the P2P sync telemetry in a UI panel; complements the banker freshness button as a second diagnostic tool accessible without slash commands
...
- [x] ~~**Remove Communication Protocol dropdown from Options**~~ **IMPLEMENTED (April 3, 2026): The "Communication Protocol" select dropdown (AUTO/Legacy Only/New Only) and its description widget were dead UI — `FEATURES.PROTOCOL_MODE` was set by the dropdown but never read by any send or receive code path. Removed the dropdown, description widget, `PROTOCOL_MODES` constant table, `PROTOCOL_MODE` field from `FEATURES`, `protocolMode` db default, and its nil-check init. Also fixed a tooltip bleed bug where the stale AceGUI description Label from this widget was bleeding its text into the Reset Database button tooltip. Locations: Options.lua; Constants.lua**
...
- [x] ~~**Request fill timestamp tooltip**~~ **IMPLEMENTED (March 30, 2026): Hovering the Date column on any request row shows a "Request Timeline" tooltip with Submitted date (grey), Filled date (green, fulfilled/complete only), or Cancelled date (red, cancelled only)**
- [x] ~~**Cancel reason dialog**~~ **IMPLEMENTED (March 30, 2026, updated April 3, 2026): Clicking Cancel on a request now opens an AceGUI reason-selection dialog instead of cancelling immediately; selected reason is stored in `req.notes` and broadcast guild-wide; cancelled rows show the reason text in the date-column tooltip below the Cancelled timestamp. Reasons are role-aware: bankers see 6 flavour-text reasons (item unavailable, policy % limit, wrong banker, already claimed by earlier requester, duplicate request, requester not in guild); non-bankers cancelling their own orders see 5 separate reasons (changed mind, found on AH, already received elsewhere, wrong item/mistake, plans changed); role check uses `TOGBankClassic_Guild:IsBank(actor)` at dialog open time so the list is always current**
