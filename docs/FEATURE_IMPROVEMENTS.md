...
- [ ] **Banker data freshness button** - Add a button that runs the equivalent of `/togbank hashdebug` and displays the results in a user-friendly popup; shows which bankers' inventory data is stale/out-of-date so non-dev users can diagnose sync issues without needing slash commands
